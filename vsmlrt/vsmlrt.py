...
def Waifu2x(
    ...
    if clip.format.sample_type != vs.FLOAT or clip.format.bits_per_sample != 32:
        raise ValueError(
            f"{func_name}: only constant format 32 bit float input supported"
        )
    ...
    if preprocess and model in (0, 1, 2):
        ...
        clip = core.resize.Bicubic(
            clip, width * 2, height * 2, filter_param_a=0, filter_param_b=0.75
        )
    ...
    backend = init_backend(
        backend=backend, channels=channels, trt_max_shapes=(tile_w, tile_h)
    )

    folder_path = os.path.join(
        models_path, "waifu2x", tuple(Waifu2xModel.__members__)[model]
    )
...
def DPIR(
    ...
    if isinstance(strength, vs.VideoNode):
        ...
        if strength.width != clip.width or strength.height != clip.height:
            raise ValueError(
                f'{func_name}: "strength" must be of the same size as "clip"'
            )
        if strength.num_frames != clip.num_frames:
            raise ValueError(
                f'{func_name}: "strength" must be of the same length as "clip"'
            )
    ...
    network_path = os.path.join(
        models_path, "dpir", f"{tuple(DPIRModel.__members__)[model]}.onnx"
    )
...
def RealESRGAN(
    ...
    if (
        not isinstance(model, int)
        or model not in RealESRGANv2Model.__members__.values()
    ):
    ...
    if model in [0, 1]:
        network_path = os.path.join(
            models_path,
            "RealESRGANv2",
            f"RealESRGANv2-{tuple(RealESRGANv2Model.__members__)[model]}.onnx".replace(
                "_", "-"
            ),
        )
    elif model == 2:
        network_path = os.path.join(
            models_path, "RealESRGANv2", "realesr-animevideov3.onnx"
        )
...
def CUGAN(
    ...
    if version == 1:
        ...
        raise ValueError(
            f"{func_name}: unknown version ({version}), must be 1 (legacy) or 2 (pro)"
        )
    ...
    if alpha != 1.0:
        ...
        alpha_constant = onnx.helper.make_node(
            "Constant", inputs=[], outputs=["alpha"], value=alpha_tensor
        )
        model.graph.node.insert(idx, alpha_constant)

        mul_node = onnx.helper.make_node(
            "Mul", inputs=[upstream_name, "alpha"], outputs=[downstream_name]
        )
...
def get_rife_input(clip: vs.VideoNode) -> typing.List[vs.VideoNode]:
    ...
    horizontal = core.std.ModifyFrame(
        empty, empty, partial(meshgrid_core, horizontal=True)
    )
    horizontal = horizontal.std.Loop(clip.num_frames)

    vertical = core.std.ModifyFrame(
        empty, empty, partial(meshgrid_core, horizontal=False)
    )
    vertical = vertical.std.Loop(clip.num_frames)

    multiplier_h = clip.std.BlankClip(
        format=vs.GRAYS, color=2 / (clip.width - 1), keep=True
    )

    multiplier_w = clip.std.BlankClip(
        format=vs.GRAYS, color=2 / (clip.height - 1), keep=True
    )
...
def RIFEMerge(
    ...
    network_path = os.path.join(
        models_path, "rife", f"rife_v{model // 10}.{model % 10}.onnx"
    )
    ...
    if scale == 1.0:
        ...
        for i in range(len(model.graph.node)):
            ...
            if (
                ...
                and node.output[0].startswith("onnx::Resize")
            ):
                resize_counter += 1
        ...
        for i in range(len(model.graph.node)):
            ...
            if (
                len(node.output) == 1
                and node.op_type == "Constant"
                and node.output[0].startswith("onnx::Mul")
            ):
...
def trtexec(
    ...
    if (
        heuristic
        and trt_version >= 850
        and core.trt.DeviceProperties(device_id)["major"] >= 8
    ):
...
def init_backend(
    backend: backendT, channels: int, trt_max_shapes: typing.Tuple[int, int]
) -> backendT:
...
def inference(
    ...
    if isinstance(backend, Backend.ORT_CPU):
        ...
        config = lambda: dict(GPU_THROUGHPUT_STREAMS=backend.num_streams)
        clip = core.ov.Model(
            clips,
            network_path,
            overlap=overlap,
            tilesize=tilesize,
            device=f"GPU.{backend.device_id}",
            builtin=False,
            fp16=backend.fp16,
            config=config,
            path_is_serialization=path_is_serialization,
